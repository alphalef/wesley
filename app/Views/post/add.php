<?php 
    ...
    switch($option) {
        case 'form':
            $option_title = "추가";
            $action = "/posts/add/".$date."/put";
            if(!isset($content)) { $content = ""; }
            $opt = "";
            break;
        case 'editform':
            $option_title = "수정";
            $action = "/posts/edit/".$date."/put/".$id;
...
<section>
    <form method="post" action="<?=$action?>" enctype="multipart/form-data" style="max-width:1000px; margin: auto">
        <div class="form-group">
            ...
            <!-- <input id="calendar" type="text" class="form-control" name="date" value="<?= substr($date, 0, 4) ?>-<?= substr($date, 4, 2) ?>-<?= substr($date, 6, 2) ?>" style="cursor:pointer"> -->
        ...
        <div style="text-align:center">
            ...
            <button type="button" class="btn btn-outline-secondary" onclick="location.href='/posts/mylist/'">글목록</button>
        </div>
    </form>
